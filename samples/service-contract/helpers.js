...
function getServiceContract(cloudHost, account, company, activity_id) {
  ...
  return new Promise(resolve => {
   ...
   const personQuery= `SELECT DISTINCT u.id,u.firstname FROM UnifiedPerson u JOIN Region r ON r.id IN u.regions JOIN Activity a ON r.externalId=a.udf.zActWLA WHERE a.id='${activity_id}'`;
  ...
  json1.data.forEach(function(currentValue){
    personsUids.set(currentValue.u.id,currentValue.u.firstname)
   });
